...
export default function JobCard({
  ...
  return (
    <div
      className={`group bg-white rounded-2xl border transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 ${
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            {/* Source badge */}
            <span
              className={`inline-block text-[11px] font-semibold border px-2 py-0.5 rounded-full mb-2 ${sourceColor}`}
            >
              {sourceLabel}
            </span>
            {/* Title */}
            <Link href={`/jobs/${job.id}`}>
              <h3 className="font-display font-bold text-navy text-base leading-snug mb-1 group-hover:text-emerald transition-colors cursor-pointer">
                {job.title}
              </h3>
            </Link>
            {/* Company */}
        ...
        {skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {skills.slice(0, 3).map((skill) => (
              <span
                key={skill}
                className="text-[11px] font-medium bg-cream text-charcoal border border-warm-gray px-2 py-0.5 rounded-md"
              >
                {skill}
              </span>
            ))}
